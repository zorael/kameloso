/++
    ...
    $(REF kameloso.plugins.common.awareness.ChannelAwareness) into your plugins.
 +/
...
/++
    ...
    Checks an internal list of channels once every $(REF dialect.defs.IRCEvent.Type.PING),
...
void startChannelQueries(ChanQueriesService service)
{
    ...
    void dg()
    {
        ...
        foreach (immutable i, immutable channelName; querylist)
        {
            ...
            // Overwrite state with $(REF ChannelState.queried);
            // $(REF ChannelState.topicKnown) etc are no longer relevant.
...
// onSelfjoin
/++
    Adds a channel we join to the internal $(REF ChanQueriesService.channels) list of
    channel states.
 +/
...
// onSelfpart
/++
    Removes a channel we part from the internal $(REF ChanQueriesService.channels)
