/++
 ...
 +  `lurk.defs.IRCEvent`s when such information is not present.
 +
 ...
 +  `lurk.defs.IRCEvent`s in the normal sense at all.
 +
...
// postprocess
/++
 +  Hijacks a reference to a `lurk.defs.IRCEvent` after parsing and
 +  fleshes out the `lurk.defs.IRCEvent.sender` and/or
 +  `lurk.defs.IRCEvent.target` fields, so that things like account names
...
// onQuit
/++
 +  Removes a user's `lurk.defs.IRCUser` entry from the `users`
...
/++
 ...
 +  change nickname, to point to the new `lurk.defs.IRCUser`.
 +
...
// PersistenceService
/++
 +  The Persistence service melds new `lurk.defs.IRCUser`s (from
 +  post-processing new `lurk.defs.IRCEvent`s) with old records of themselves.
 +
 +  Sometimes the only bit of information about a sender (or target) embedded in
 +  an `lurk.defs.IRCEvent` may be his/her nickname, even though the
 +  event before detailed everything, even including their account name. With
 +  this service we aim to complete such `lurk.defs.IRCUser` entries as
