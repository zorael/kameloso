...
public:
    ...
    /// Whether or not broadcasters are always implicitly class $(REF dialect.defs.IRCUser.Class.staff).
...
/++
    ...
    $(REF TwitchBotSettings.bellOnImportant) setting is set.
 +/
@Chainable
...
// onSelfjoin
/++
    Registers a new $(REF TwitchBotPlugin.Room) as we join a channel, so there's
    always a state struct available.

    Simply passes on execution to $(REF handleSelfjoin).
 +/
...
/++
    ...
    Creates the timer $(REF core.thread.fiber.Fiber)s that there are definitions for in
    $(REF TwitchBotPlugin.timerDefsByChannel).

    Params:
        plugin = The current $(REF TwitchBotPlugin).
...
// onSelfpart
/++
    Removes a channel's corresponding $(REF TwitchBotPlugin.Room) when we leave it.
...
/++
    ...
    Changes are persistently saved to the $(REF TwitchBotPlugin.bannedPhrasesFile) file.
 +/
@(IRCEvent.Type.CHAN)
@(IRCEvent.Type.SELFCHAN)
@(PrivilegeLevel.operator)
...
/++
    ...
    Changes are persistently saved to the $(REF TwitchBotPlugin.timersFile) file.
 +/
@(IRCEvent.Type.CHAN)
@(IRCEvent.Type.SELFCHAN)
@(PrivilegeLevel.operator)
...
/++
    ...
    sense to automate $(REF onCommandStop).
 +/
@(ChannelPolicy.home)
...
/++
    ...
    Params:
        plugin = The current $(REF TwitchBotPlugin).
        room = The $(REF TwitchBotPlugin.Room) of the channel.
...
/++
    ...
    It uses a simple state machine in $(REF kameloso.common.findURLs). If the Webtitles
    ...
    any user who has been given a temporary permit via $(REF onCommandPermit).
...
/++
    ...
    * Bells on any message, if the $(REF TwitchBotSettings.bellOnMessage) setting is set.
    ...
    * Bumps the message counter for the channel, used by timers.
    ...
    Note: This is annotated $(REF kameloso.plugins.common.core.Terminating) and must be
    placed after all other handlers with these $(REF dialect.defs.IRCEvent.Type) annotations.
...
// onMyInfo
/++
    Sets up a Fiber to periodically call timer $(REF core.thread.fiber.Fiber)s with a
    periodicity of $(REF TwitchBotPlugin.timerPeriodicity).

    Cannot be done on $(REF dialect.defs.IRCEvent.Type.RPL_WELCOME) as the server
...
/++
    ...
    $(REF TwitchBotSettings.enableAPIFeatures) into $(REF TwitchBotPlugin) so it can be
...
package:
    ...
    static struct Room
    {
        ...
        /// Timer $(REF core.thread.fiber.Fiber)s.
        Fiber[] timers;
    ...
    /++
        ...
        to doing nothing when $(REF TwitchbotSettings.enabled) is false.
    ...
    // onEvent
    /++
        Override $(REF kameloso.plugins.common.core.IRCPluginImpl.onEvent) and inject a server check, so this
        plugin does nothing on non-Twitch servers. Also filters $(REF dialect.defs.IRCEvent.Type.CHAN)
        events to only trigger on active channels (that have its $(REF TwitchBotPlugin.Room.enabled)
        set to true).
