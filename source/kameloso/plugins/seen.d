/++
    ...
    Callback delegates and $(REF core.thread.fiber.Fiber)s *are* supported. They can
...
// We need the definition of an $(REF kameloso.plugin.core.IRCPlugin) and other crucial things.
...
// Likewise $(REF dialect.defs), for the definitions of an IRC event.
private import dialect.defs;

// $(REF kameloso.irccolours) for some IRC colouring and formatting.
private import kameloso.irccolours : ircBold, ircColourByHash;

// $(REF kameloso.common) for some globals.
private import kameloso.common : Tint, logger;

// $(REF std.datetime.systime) for the $(REF std.datetime.systime.Clock), to update times with.
private import std.datetime.systime : Clock;

// $(REF std.typecons) for $(REF std.typecons.Flag) and its friends.
private import std.typecons : Flag, No, Yes;


/+
    Most of the module can (and ideally should) be kept private. Our surface
    area here will be restricted to only one $(REF kameloso.plugins.common.core.IRCPlugin)
...
/++
    ...
    $(REF SeenSettings) and its $(REF kameloso.plugins.common.core.IRCPluginState).

    The $(REF kameloso.plugins.common.core.IRCPluginState) is a struct housing various
    ...
    later with the $(REF kameloso.plugins.common.core.IRCPluginImpl) mixin.
    ...
    struct IRCPluginState
    {
        ...
        ScheduledFiber[] scheduledFibers;  // `ScheduledFiber` is a struct in $(REF kameloso.thread)
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.client) houses information about
        the client itself, such as your nickname and other things related to an
        IRC client.

    * $(REF kameloso.plugins.common.core.IRCPluginState.server) houses information about
        the server you're connected to.

    * $(REF kameloso.plugins.common.core.IRCPluginState.bot) houses information about
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.settings) is a copy of the
        "global" $(REF kameloso.kameloso.CoreSettings), which contains information
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.connSettings) is like `settings`,
        except for values relating to the connection to the server; whether
        to use IPv6, paths to any certificates, and the such.

    * $(REF kameloso.plugins.common.core.IRCPluginState.mainThread) is the *thread ID* of
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.users) is an associative array
        keyed with users' nicknames. The value to that key is an
        $(REF dialect.defs.IRCUser) representing that user in terms of nickname,
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.channels) is another associative
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.replays) is also an
        associative array into which we place $(REF kameloso.plugins.common.core.Replay)s.
        The main loop will pick up on these and call WHOIS on the nickname in the key.
        A $(REF kameloso.plugins.common.core.Replay) is otherwise just an
        $(REF dialect.defs.IRCEvent) to be played back when the WHOIS results
        return, as well as a function pointer to call with that event. This is
        all wrapped in a function $(REF kameloso.plugins.common.base.enqueue), with the
        queue management handled behind the scenes.

    * $(REF kameloso.plugins.common.core.IRCPluginState.hasReplays) is merely a bool
        of whether or not there currently are any $(REF kameloso.plugins.common.core.Replay)s
        in $(REF kameloso.plugins.common.core.IRCPluginState.replays), cached to avoid
        associative array length lookups.

    * $(REF kameloso.plugins.common.core.IRCPluginState.repeats) is an array of
        $(REF kameloso.plugins.common.core.Repeat)s, which is instrumental in repeating
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.awaitingFibers) is an associative
        array of $(REF core.thread.fiber.Fiber)s keyed by $(REF kameloso.ircdefs.IRCEvent.Type)s.
    ...
    * $(REF kameloso.plugins.common.core.IRCPluginState.awaitingDelegates) is literally
        an array of callback delegates, to be triggered when an event of a
        matching type comes along.

    * $(REF kameloso.plugins.common.core.IRCPluginState.scheduledFibers) is also an array of
        $(REF core.thread.fiber.Fiber)s, but not an associative one keyed on event types.
        Instead they are tuples of a $(REF core.thread.fiber.Fiber) and a `long`
        timestamp of when they should be run.
        Use $(REF kameloso.plugins.common.delayFiber) to enqueue, or
        $(REF kameloso.plugins.common.delayFiberMsecs) for greater granularity.

    * $(REF kameloso.plugins.common.core.IRCPluginState.scheduledDelegates) is likewise an
        array of delegates, to be triggered at a later point in time.

    * $(REF kameloso.plugins.common.core.IRCPluginState.nextScheduledTimestamp) is also a
        UNIX timestamp, here of when the next $(REF kameloso.thread.ScheduledFiber) in
        $(REF kameloso.plugins.common.core.IRCPluginState.scheduledFibers) *or* the next
        $(REF kameloso.thread.ScheduledDelegate) in
        $(REF kameloso.plugins.common.core.IRCPluginState.scheduledDelegates) is due to be
        processed. Caching it here means we won't have to go through the arrays
        to find out as often.

    * $(REF kameloso.plugins.common.core.IRCPluginState.updateSchedule) merely iterates all
        scheduled fibers and delegates, caching the time at which the next one
        should trigger.

    * $(REF kameloso.plugins.common.core.IRCPluginState.botUpdated) is set when
        $(REF kameloso.plugins.common.core.IRCPluginState.bot) was updated during parsing
        and/or postprocessing. It is merely for internal use.

    * $(REF kameloso.plugins.common.core.IRCPluginState.clientUpdated) is likewise set when
        $(REF kameloso.plugins.common.core.IRCPluginState.client) was updated during parsing
        and/or postprocessing. Ditto.

    * $(REF kameloso.plugins.common.core.IRCPluginState.serverUpdated) is likewise set when
        $(REF kameloso.plugins.common.core.IRCPluginState.server) was updated during parsing
        and/or postprocessing. Ditto.

    * $(REF kameloso.plugins.common.core.IRCPluginState.settingsUpdated) is likewise set when
        $(REF kameloso.plugins.common.core.IRCPluginState.settings) was updated during parsing
        and/or postprocessing. Ditto.

    * $(REF kameloso.plugins.common.core.IRCPluginState.abort) is a pointer to the global
...
private:  // Module-level private.
    ...
    /++
        ...
        $(REF kameloso.plugins.common.core.IRCPlugin). They don't do much by themselves
        other than call the module's functions.

        As an exception, it mixes in the bits needed to automatically call
        functions based on their $(REF dialect.defs.IRCEvent.Type) annotations.
    ...
    /++
        ...
        $(REF kameloso.messaging) functions, *partially applied* with the main thread ID,
...
/++
    ...
    our configurable bits, which we already made an instance of in $(REF SeenPlugin).

    If it's annotated with $(REF kameloso.plugins.common.core.Settings), the wizardry will
    ...
    header in the file. Hence, this plugin's $(REF SeenSettings) will get the header
    `[Seen]`.
 +/
@Settings struct SeenSettings
{
    /++
        ...
        The @$(REF kameloso.plugins.common.core.Enabler) annotation makes it special and
...
/++
    ...
    This function will be called whenever an $(REF dialect.defs.IRCEvent) is
    being processed of the $(REF dialect.defs.IRCEvent.Type)s that we annotate
    the function with.

    The $(REF kameloso.plugins.common.core.Chainable) annotations mean that the plugin
    will also process other functions in this module with the same
    $(REF dialect.defs.IRCEvent.Type) annotations, even if this one matched. The
    ...
    $(REF kameloso.plugins.common.core.Chainable) only for the modules and functions that
    actually need it.

    The $(REF kameloso.plugins.common.core.ChannelPolicy) annotation dictates whether or not this
    function should be called based on the *channel* the event took place in, if
    applicable. The two policies are $(REF kameloso.plugins.common.core.ChannelPolicy.home),
    in which only events in channels in the $(REF kameloso.kameloso.IRCBot.homeChannels)
    array will be allowed to trigger this; or $(REF kameloso.plugins.common.core.ChannelPolicy.any),
    in which case anywhere goes. For events that don't correspond to a channel (such as
    $(REF dialect.defs.IRCEvent.Type.QUERY)) the setting is ignored.

    The $(REF kameloso.plugins.common.core.PrivilegeLevel) annotation dictates who is
    authorised to trigger the function. It has six policies, in increasing
    order of importance:
    $(REF kameloso.plugins.common.core.PrivilegeLevel.ignore),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.anyone),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.registered),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.whitelist),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.operator),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.staff) and
    $(REF kameloso.plugins.common.core.PrivilegeLevel.admin).

    * $(REF kameloso.plugins.common.core.PrivilegeLevel.ignore) will let precisely anyone
        trigger it, without looking them up.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.anyone) will let precisely anyone
        trigger it, but only after having looked them up.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.registered) will let anyone logged
        into a services account trigger it.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.whitelist) will only allow users
        in the whitelist section of the `users.json` resource file. Consider this
        to correspond to "regulars" in the channel.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.operator) will only allow users
        in the operator section of the `users.json` resource file. Consider this
        to correspond to "moderators" in the channel.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.staff) will only allow users
        in the staff section of the `users.json` resource file. Consider this
        to correspond to channel owners.<br>
    * $(REF kameloso.plugins.common.core.PrivilegeLevel.admin) will allow only you and
        your other superuser administrators, as defined in the configuration file.

    In the case of $(REF kameloso.plugins.common.core.PrivilegeLevel.whitelist),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.operator),
    $(REF kameloso.plugins.common.core.PrivilegeLevel.staff) and
    $(REF kameloso.plugins.common.core.PrivilegeLevel.admin) it will look you up and
    compare your *services account name* to those known good before doing
    anything. In the case of $(REF kameloso.plugins.common.core.PrivilegeLevel.registered),
    merely being logged in is enough. In the case of
    $(REF kameloso.plugins.common.core.PrivilegeLevel.anyone), the WHOIS results won't
...
void onSomeAction(SeenPlugin plugin, const ref IRCEvent event)
{
    /+
        ...
        $(REF dialect.defs.IRCEvent.Type)s it is annotated with. Furthermore, it will
...
/++
    ...
    $(REF dialect.defs.IRCEvent.Type.QUIT) events don't carry a channel.
    ...
    $(REF dialect.defs.IRCEvent.Type.RPL_NAMREPLY) lists were given when
...
/++
    ...
    Like $(REF dialect.defs.IRCEvent.Type.QUIT),
    $(REF dialect.defs.IRCEvent.Type.NICK) events don't carry a channel, so we
    can't annotate it $(REF kameloso.plugins.common.core.ChannelPolicy.home); all we know
    ...
    $(REF dialect.defs.IRCEvent.Type.RPL_NAMREPLY) lists on join).
 +/
@Chainable
@(IRCEvent.Type.NICK)
@(PrivilegeLevel.ignore)
...
/++
    ...
    $(REF kameloso.plugins.chanqueries.ChanQueriesService) services instigates this
...
void onNamesReply(SeenPlugin plugin, const ref IRCEvent event)
{
    ...
    /+
        Use a $(REF std.algorithm.iteration.splitter) to iterate each name and call
        $(REF updateUser) to update (or create) their entry in the
        $(REF SeenPlugin.seenUsers) associative array.
     +/
...
// onCommandSeen
/++
    Whenever someone says "seen" in a $(REF dialect.defs.IRCEvent.Type.CHAN) or
    a $(REF dialect.defs.IRCEvent.Type.QUERY), and if
    $(REF dialect.defs.IRCEvent.Type.CHAN) then only if in a *home*, this function triggers.

    The $(REF kameloso.plugins.common.core.BotCommand) annotation defines a piece of text
    that the incoming message must start with for this function to be called.
    $(REF kameloso.plugins.common.core.PrefixPolicy) deals with whether the message has to
    ...
    * `nickname`, where the message has to start with bot's nickname:
        "`kameloso: seen`" -- except if it's in a $(REF dialect.defs.IRCEvent.Type.QUERY) message.<br>
    ...
    $(REF kameloso.plugins.common.core.BotCommand). It will since have been sliced off,
    so we're left only with the "arguments" to "`seen`". $(REF dialect.defs.IRCEvent.aux)
    contains the triggering word, if it's needed.

    If this is a $(REF dialect.defs.IRCEvent.Type.CHAN) event, the original lines
    ...
    Lastly, the $(REF kameloso.plugins.common.core.Description) annotation merely defines
    how this function will be listed in the "online help" list, shown by triggering
    the $(REF kameloso.plugins.help.HelpPlugin)'s' "`help`" command.
 +/
@(IRCEvent.Type.CHAN)
@(IRCEvent.Type.QUERY)
...
void onCommandSeen(SeenPlugin plugin, const ref IRCEvent event)
{
    ...
    /+
        ...
        $(REF kameloso.messaging), and additionally this module has mixed in
        `MessagingProxy` in the $(REF SeenPlugin), creating even shorter shorthand
        versions.
...
/++
    ...
    saves seen users to disk once every $(REF SeenPlugin.timeBetweenSaves) seconds.
...
// onBusMessage
/++
    Receives a passed $(REF kameloso.thread.BusMessage) with the "`seen`" header,
    ...
    Params:
        plugin = The current $(REF SeenPlugin).
...
/++
    $(REF kameloso.plugins.common.awareness.UserAwareness) is a mixin template; a few functions
    defined in $(REF kameloso.plugins.common.awareness) to deal with common bookkeeping that
...
/++
    Complementary to $(REF kameloso.plugins.common.awareness.UserAwareness) is
    $(REF kameloso.plugins.common.awareness.ChannelAwareness), which will add in bookkeeping
