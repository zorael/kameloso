...
// Likewise `lurk.defs`, for the definitions of an IRC event.
import lurk.defs;
...
/++
 ...
 +     `lurk.defs.IRCUser` representing that user in terms of nickname,
 ...
 +     `lurk.defs.IRCEvent` to be played back when the `WHOIS` results
...
private:  // Module-level private.
    ...
    /++
     ...
     +  functions based on their `lurk.defs.IRCEvent.Type` annotations.
...
/++
 ...
 +  current time.
 +
 +  This function will be called whenever an `lurk.defs.IRCEvent` is
 +  being processed of the `lurk.defs.IRCEvent.Type`s that we annotate
 +  the function with.
 +
 ...
 +  `lurk.defs.IRCEvent.Type` annotations, even if this one matched. The
 ...
 +  in which only events in channels in the `lurk.common.IRCClient.homes`
 ...
 +  `lurk.defs.IRCEvent.Type.QUERY`) the setting is ignored.
 +
...
void onSomeAction(SeenPlugin plugin, const IRCEvent event)
{
    /+
        ...
        This will, as such, be automatically called on `lurk.defs.IRCEvent.Type.EMOTE`,
        `lurk.defs.IRCEvent.Type.QUERY`, `lurk.defs.IRCEvent.Type.CHAN`,
        `lurk.defs.IRCEvent.Type.JOIN`, and `lurk.defs.IRCEvent.Type.PART`
...
/++
 ...
 +
 +  `lurk.defs.IRCEvent.Type.QUIT` events don't carry a channel.
 ...
 +  `lurk.defs.IRCEvent.Type.RPL_NAMREPLY` lists were given when
...
/++
 ...
 +
 +  Like `lurk.defs.IRCEvent.Type.QUIT`,
 +  lurk.defs.IRCEvent.Type.NICK` events don't carry a channel, so we
 ...
 +  `lurk.defs.IRCEvent.Type.RPL_NAMREPLY` lists on join).
 +/
@(Chainable)
@(IRCEvent.Type.NICK)
@(PrivilegeLevel.ignore)
...
// onCommandSeen
/++
 +  Whenever someone says "seen" in a `lurk.defs.IRCEvent.Type.CHAN` or
 +  a `lurk.defs.IRCEvent.Type.QUERY`, and if
 +  `lurk.defs.IRCEvent.Type.CHAN` then only if in a *home*, processes this function.
 +
 ...
 +     if it's in a `lurk.defs.IRCEvent.Type.QUERY` message.<br>
 +
 ...
 +
 +  If this is a `lurk.defs.IRCEvent.Type.CHAN` event, the original lines
 +  could (for example) have been "`kameloso: seen Joe`", or merely "`!seen Joe`"
 +  (assuming a "`!`" prefix). If it was a private `lurk.defs.IRCEvent.Type.QUERY`
 ...
 +
 +  As a result, the `lurk.defs.IRCEvent` `event` would look something
 +  like this:
 +
 +  ---
