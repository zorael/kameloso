    ...
    while (true)
    {
        ...
        void eventToServer(Message m) scope
        {
            ...
            case MODE:
                ...
                line = format(pattern, m.event.channel, m.event.auxstrings[0], m.event.content.strippedRight);
                break;
            ...
            case JOIN:
                if (m.event.auxstrings[0].length)
                {
                    // Key, assume only one channel
                    line = text("JOIN ", m.event.channel, ' ', m.event.auxstrings[0]);
                }
                else
                {
                    prelude = "JOIN ";
