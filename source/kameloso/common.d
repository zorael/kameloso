...
// logger
/++
    Instance of a [kameloso.logger.KamelosoLogger|KamelosoLogger], providing timestamped and
    coloured logging.
...
// initLogger
/++
    Initialises the [kameloso.logger.KamelosoLogger|KamelosoLogger] logger for use in this thread.

    It needs to be separately instantiated per thread, and even so there may be
    race conditions. Plugins are encouraged to use [kameloso.thread.ThreadMessage|ThreadMessage]s
...
/++
    ...
    Params:
        now = A [std.datetime.systime.SysTime|SysTime] of the base date from which to proceed
...
// curlErrorStrings
/++
    Reverse mapping of [etc.c.curl.CurlError|CurlError] member values to their string names.
