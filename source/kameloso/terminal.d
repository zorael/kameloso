...
// colour
/++
    Takes a mix of a $(REF TerminalForeground), a $(REF TerminalBackground), a
    $(REF TerminalFormat) and/or a $(REF TerminalReset) and composes them into a single
    terminal format code token. Overload that creates an $(REF std.array.Appender)
...
/++
    Takes a mix of a $(REF TerminalForeground), a $(REF TerminalBackground), a
    $(REF TerminalFormat) and/or a $(REF TerminalReset) and composes them into a format code token.
...
// colourByHash
/++
    Hashes the passed string and picks a $(REF TerminalForeground) colour by modulo.
    Overload that takes an array of $(REF TerminalForeground)s, to pick between.

    Params:
        word = String to hash and base colour on.
        fgArray = Array of $(REF TerminalForeground)s to pick a colour from.

    Returns:
        A $(REF TerminalForeground) based on the passed string, picked from the
...
/++
    ...
    Returns:
        A $(REF TerminalForeground) based on the passed string.
 +/
version(Colours)
